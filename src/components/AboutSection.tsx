...
const AboutSection = () => {
  return (
    <section id="about" className="py-20 md:py-28 relative overflow-hidden bg-white">
      {/* Ambient gold glow */}
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-[700px] h-[700px] rounded-full bg-accent/[0.06] blur-[160px] pointer-events-none" />

      <div className="container mx-auto px-6 max-w-5xl relative z-10">
        <ScrollReveal>
          <div className="max-w-2xl mx-auto text-center mb-16">
            <h3 className="font-heading text-2xl md:text-3xl font-bold text-accent mb-4">Who We Are</h3>
            <h2 className="font-heading text-3xl md:text-5xl font-bold text-primary mb-5 leading-[1.15]">
        ...
        {/* Vision & Mission */}
        <div className="grid md:grid-cols-2 gap-px bg-primary/[0.08] rounded-lg overflow-hidden mb-16 border border-primary/10">
          <ScrollReveal delay={0.1}>
            <div className="bg-white p-8 md:p-10 h-full">
              ...
              <p className="text-primary/80 leading-[1.8] text-[17px] pl-[52px]">
        ...
        <ScrollReveal delay={0.15}>
          <div className="text-center">
            ...
            <h4 className="font-heading text-xl font-bold text-primary mb-6">What Guides Us</h4>
            <div className="flex flex-wrap justify-center gap-3 mb-10">
              {values.map((v) => (
                <div
                  key={v.label}
                  className="inline-flex items-center gap-2.5 bg-primary/[0.04] rounded-md px-5 py-2.5 border border-primary/15 hover:border-accent/60 transition-all duration-400"
                >
                  <v.icon className="w-3.5 h-3.5 text-accent" />
                  <span className="text-sm font-medium text-primary/85">{v.label}</span>
                </div>
              ))}
            </div>
            <Button asChild variant="outline" className="font-sans font-semibold text-sm rounded-md tracking-wide bg-transparent border-primary/30 text-primary hover:bg-accent hover:text-accent-foreground hover:border-accent">
